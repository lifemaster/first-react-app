...
class Slider extends React.Component {
  ...
  intervalBetweenSlides = () => {
    if (this.state.autoplay) {
      this.setState({ active: (this.state.active === this.state.max - 1) ? 0 : this.state.active + 1 });
    }
  }
  ...
  nextOne = () => {
    this.setState({ active: (this.state.active < this.state.max - 1) ? this.state.active + 1 : 0 });
  }

  prevOne = () => {
    this.setState({ active: (this.state.active > 0) ? this.state.active - 1 : this.state.max - 1 });
  }
